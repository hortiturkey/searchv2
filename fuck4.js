...
$('#searchv2-input-2').on('typeahead:beforeclose', function (event, data) {
    event.preventDefault()
});
$('#searchv2-input-m-2').on('typeahead:beforeclose', function (event, data) {
    event.preventDefault()
});
...

        {
            ...
            templates: {
                ...
                suggestion: function (data) {
                    ...
                    return `<a href="${data.slug}" class="search2-result-product w-inline-block"><img src="${data.imageURL}" loading="lazy" id="w-node-9224dd65e772-4dedb92e" alt class="search2-result-product-image"><div><div class="post-category-container"><div class="post-tag-colored prod">ÜRÜN</div><div class="post-tag-dot"></div><div class="post-tag no-second-line">${data.subCategory}</div></div><h4 class="search2-result-title no-second-line">${data.title}</h4><div class="post-category-container"><div class="post-tag">${data.company}</div></div></div></a>`;
