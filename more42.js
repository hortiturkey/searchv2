...
$('#searchv2-input-m').keypress(function(e){
    if(e.which == 13){
        callajax();
        $(this).blur();    
    }
...
$('#searchv2-input').keypress(function(e){
    if(e.which == 13){
        callajax();
        $(this).blur();    
    }
...
async function getSearchDocs() {
    const userloggedin = await AuthPromise;
    const snapshot = await db.collection('search').doc('searchArray').get();
    const searchObject = snapshot.data();
    const searchTempList = Object.values(searchObject);
...
Promise.all([getSearchDocs()]).then(function () {
    ...
    $('.typeahead').typeahead({
            ...
            highlight: true,
            hint: false,
            menu: $('#searchv2-desktop-main-container'),
        },
...

        {
            ...
            templates: {
                header: function (context) {
                    return `<div class="search2-tt-header"> <h3 class="search2-tt-header-title">Ürünler(${suggestions.length})</h3><a href="#" class="search2-tt-header-link-box">Tümünü Gör →</a></div>`;
                },
                header: `<div class="search2-tt-header"> <h3 class="search2-tt-header-title">Ürünler(${suggestions.length})</h3><a href="#" class="search2-tt-header-link-box">Tümünü Gör →</a></div>`,
                suggestion: function (data) {
